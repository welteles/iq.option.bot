...
export class IndicatorService {
    ...
    public checkIndicator(candles: Core.ICandle): void {
        ...
        this.technicalIndicators = [];
        this.movingAverage = [];
        Core.logger().info(`${Core.timestampHelper()} datetime`);
        for (const indicator of this.indicators) {
            const condition = indicator.checkCondition(candles);
            if (
                indicator.constructor.name === "IndicatorEMA" ||
                indicator.constructor.name === "IndicatorMA"
            ) {
        ...
        if (
            Core.global.config.indicator.strongOnly &&
            IndicatorService.force === Core.StrategySide.WEAK
        ) {
        ...
        if (
            (indIndication === Core.StrategySide.STRONG_BUY &&
                maIndication === Core.StrategySide.BUY) ||
            (maIndication === Core.StrategySide.STRONG_BUY &&
                indIndication === Core.StrategySide.BUY) ||
            (maIndication === Core.StrategySide.STRONG_BUY &&
                indIndication === Core.StrategySide.STRONG_BUY)
        ) {
        ...
        if (
            (indIndication === Core.StrategySide.STRONG_SELL &&
                maIndication === Core.StrategySide.SELL) ||
            (maIndication === Core.StrategySide.STRONG_SELL &&
                indIndication === Core.StrategySide.SELL) ||
            (maIndication === Core.StrategySide.STRONG_SELL &&
                indIndication === Core.StrategySide.STRONG_SELL)
        ) {
